...
(function () {
    ...
    makeWatchFor = function (directive) {
        var mask;
    ...
    create = function (persist) {
        ...
        watch = function (directive, path, options) {
            ...
            if (!args[0] || !args[0]) {
              throw new Error("Usage: Object.watch({'inotify_flag': function () {}}, '/path/to/watch');");
            }

            wd = inotify.addWatch({ 
                path: path,
                callback: makeCallback(directive),
                watch_for: makeWatchFor(directive)
            });
